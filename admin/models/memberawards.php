...
class jSchuetzeModelMemberawards extends JModelList
{
    ...
    public function __construct($config = array())
    {
        if (empty($config['filter_fields'])) {
            $config['filter_fields'] = array('id', 'fk_mitglied', 'fk_auszeichnung', 'auszeichnungsdatum', 'periode', 'foto_url');
        }
	...
	protected function getListQuery()
	{
        ...
        $query->join('', '#__jschuetze_auszeichnungen AS award  ON (aus.fk_auszeichnung = award.id)');
        $query->join('', '#__jschuetze_mitglieder     AS member ON (aus.fk_mitglied     = member.id)');
